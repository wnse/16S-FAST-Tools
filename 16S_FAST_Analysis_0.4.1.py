...
output_dir = '/mnt/data/work/test_16S_FAST_v4/test_out_LJEQYPE'
...
args = parser.parse_args(['-ar1',os.path.join(raw_data_dir,'LJ-EQYPE-P_S35_L008_R1_001.fastq.gz'),
                          '-ar2',os.path.join(raw_data_dir,'LJ-EQYPE-P_S35_L008_R2_001.fastq.gz'),
                          '-lr1',os.path.join(raw_data_dir,'LJ-EQYPE-L_S36_L008_R1_001.fastq.gz'),
                          '-lr2',os.path.join(raw_data_dir,'LJ-EQYPE-L_S36_L008_R2_001.fastq.gz'), 
                          '-o',output_dir,
                          '-pc','6',
                          '-c','24',
                          '-minl','1200',
...
merge_uchimeout = os.path.join(result_dir,'merged.uchimeout.out.txt')
merge_ch_fa = os.path.join(result_dir,'merged.ch.fasta')
merge_nonch_fa = os.path.join(result_dir,'merged.nonch.fasta')
ID_info = os.path.join(result_dir ,'asv.id.info')
asv_fa = os.path.join(result_dir,'asv.fasta')
asv_fa_clstr_table = os.path.join(result_dir,'asv.clstr.info')
#合并组装后的contigs
assemble_list = []
...
tmp, df_cut_info = cut_fa_by_len.cut_fa_by_len(merge_trim_fa2,
                                               merge_filter_fa,
                                               args.minlength,
                                               args.maxlength)
...
#去除嵌合
remove_chimeras_uchime.remove_chimeras_uchime(merge_filter_fa,
                                              merge_uchimeout,
                                              merge_ch_fa,
                                              merge_nonch_fa,
                                              usearch)
#聚类
#100%相似度 cd-hit
submit_cdhit.submit_cdhit(merge_nonch_fa, asv_fa, 1, cdhit, threads)
#去除同一umiID中未聚类在同一组的contig
rep_seq_tab = get_consensus_seq_from_cdhit.get_consensus_seq_from_cdhit(asv_fa+'clstr', 
                                                          merge_nonch_fa, 
                                                          asv_fa, 
                                                          args.name)
pd.DataFrame(rep_seq_tab,
             columns=(['asv_id','size','seq_id'])).to_csv(asv_fa_clstr_table, 
                                                              sep='\t',
                                                              index=False)
#物种分类
