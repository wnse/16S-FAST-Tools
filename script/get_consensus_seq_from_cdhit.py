...
def get_consensus_seq_from_cdhit(clstr,fa,outfa,tag):
    ...
    for name, cl in seq2cl.items():
        ...
        if len(set(cl_s)) == 1:
            ...
            print('unconsensus seq {} in cluster {}'.format(name,set(cl_s)))
    ...
    for i, size in sorted(rep_seq_size.items(), key=lambda d:d[1], reverse=True):
        ...
        new_id = tag + '_' + str(n)
        seq.id = new_id
        seq.description = i + ';size=' + str(size)
        id2id[i] = new_id
        rep_seq.append(seq)
        rep_seq_tab.append([new_id, size,'|'.join(rep_seq_seqs[i])])
        n+=1
    count = SeqIO.write(rep_seq,outfa,'fasta')
    logging.info('Total No. of consensus asv seq: {}'.format(count))
    return rep_seq_tab
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format='%(asctime)s %(levelname)s:%(message)s',
                        level='INFO')
    clstr = '../../test_16S_FAST_v4/test_out/test/asv.fasta.clstr'
    fa = '../../test_16S_FAST_v4/test_out/test/merged.filter.fasta'
    outfa = '../../test_16S_FAST_v4/test_out/test/merged.asv.fasta'
    sample_name = 'test'
    outtab = ''
    if args.outtab:
        outtab = args.outtab
    ...
    rep_seq_tab = get_consensus_seq_from_cdhit(args.inclstr, 
                                               args.infasta, 
                                               args.outfasta, 
                                               args.name)
